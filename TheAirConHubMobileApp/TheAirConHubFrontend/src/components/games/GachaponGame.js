...
const gachaponRewards = [
  { value: 0, color: "#6b7280", weight: 40 },
  { value: 10, color: "#3b82f6", weight: 25 },
  { value: 15, color: "#f97316", weight: 15 },
  { value: 20, color: "#22c55e", weight: 10 },
  { value: 30, color: "#ef4444", weight: 5 },
  { value: 40, color: "#a855f7", weight: 3 },
  { value: 50, color: "#ec4899", weight: 2 },
  { value: 100, color: "#14b8a6", weight: 1 },
];
  ...
  const handleSpin = () => {
    ...
    setTimeout(() => {
      ...
      Animated.timing(knobRotation, {
        ...
        const totalWeight = gachaponRewards.reduce(
          (sum, item) => sum + item.weight,
          0
        );
        let randomNum = Math.random() * totalWeight;
        let selectedReward = gachaponRewards[0];

        for (let i = 0; i < gachaponRewards.length; i++) {
          if (randomNum < gachaponRewards[i].weight) {
            selectedReward = gachaponRewards[i];
            break;
          }
          randomNum -= gachaponRewards[i].weight;
        }
  ...
  return (
    <View style={localStyles.gameCard}>
      ...
      <Text style={localStyles.gameSubtitle}>
        {gameState === "dispensed"
          ? "You won a prize!"
          : "Test your luck for bonus points!"}
      </Text>
        ...
        <View style={localStyles.machineWrapper}>
          ...
          {gameState === "dispensed" && reward && (
            <View style={localStyles.rewardOverlay}>
              <View
                style={[localStyles.pixelBox, { borderColor: reward.color }]}
              >
                <Text style={[localStyles.pixelText, { color: reward.color }]}>
                  {reward.value}
                </Text>
                <Text
                  style={[localStyles.pixelSubText, { color: reward.color }]}
                >
                  POINTS
                </Text>
              </View>
